...
import csv, math, os, re, unicodedata as ucd
...
def _candidate_paths() -> List[Path]:
    ...
    cands += [here/"data"/"nrc_words.csv", cwd/"data"/"nrc_words.csv", here/"nrc_words.csv", cwd/"nrc_words.csv"]
    seen, out = set(), []
    for p in cands:
        rp = p.resolve()
        if rp not in seen:
...
class LexiconScorer:
    ...
    def from_csv(
        ...
        with open(p, "r", encoding="utf-8-sig", newline="") as f:
            ...
            for row in reader:
                ...
                sval = ("" if row.get(scol) is None else str(row.get(scol))).strip().replace(",", ".")
                try:
                    s = float(sval)
                except Exception:
                    continue
        ...
        min_v = min(vocab.values()); max_v = max(vocab.values())
        if debug or _dbg_on():
    ...
    def _norm_score(self, x: float) -> float:
        if self.min_v < 0.0 or self.max_v > 1.0:
            if self.max_v == self.min_v: return 0.0
    ...
    def sincerity(self, text: str, mode: str = "all", alpha: float = 2.0) -> Tuple[float, int, int, float]:
        ...
        toks_raw = self._tokenize(text)
        if not toks_raw: return 0.0, 0, 0, 0.0
        toks = [self._norm_token(t) for t in toks_raw]
        total = len(toks)
...
def sincerity_score(text: str, mode: str = "all", alpha: float = 2.0) -> Tuple[float, int, int, float]:
